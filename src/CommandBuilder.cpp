...
CommandPlan CommandBuilder::build(const PhysicalDisk& disk, const PartitionPlan& plan) {
    ...
    cp.commands.push_back("# Uruchom cmd/PowerShell jako Administrator");
    cp.commands.push_back("# diskpart /s <wygenerowany_plik.txt>");
    cp.commands.push_back("# Przykładowy skrypt diskpart:");
    cp.commands.push_back("select disk X");
